...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            '  python3 csvTrim.py --input ./csv_folder --output trimmed.csv --excel\n\n'
            ...
            '  python3 csvTrim.py --preset-save MyPreset \\\n'
            "    --filter \"['Compute']\" --filter-column serviceFamily\n\n"
            '  # Copy an existing preset under a new name:\n'
            '  python3 csvTrim.py --preset Azure --preset-save AzureCopy\n'
        )
    )
    parser.add_argument('--version',       '-v',  action='version', version=f'csvTrim {VERSION}')
    ...
    parser.add_argument('--filter-column', '-fc',
                        default=None,
                        metavar='COL',
                        help=(
                            "Column to filter values against. "
                            "Omit to use the default preset."
                        ))
    ...
    parser.add_argument('--preset-file',   '-pf',
                        default=None,
                        metavar='FILE',
                        help=(
    ...
    parser.add_argument('--preset-save',   '-ps',
                        default=None,
                        metavar='NAME',
                        help=(
    ...
    if args.preset is not None:
        ...
        filter_vals = (parse_list_arg(args.filter, '--filter',
        ...
        columns     = (parse_list_arg(args.columns, '--columns', "\"['meterCategory', 'quantity']\"")
    ...
    print()
    print(f"  csvTrim {VERSION}  ·  {input_label}  ·  {total_files} file{'s' if total_files > 1 else ''}{preset_label}")
    print(f"  {'─' * (W - 4)}")
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        try:
            ...
            file_header = pd.read_csv(csv_file, nrows=0, encoding='utf-8',
            ...
            if missing_cols:
                skipped.append((filename, f"columns not found, skipped: {missing_cols}"))
            ...
            for chunk in pd.read_csv(csv_file, chunksize=100_000,
                         ...
                         low_memory=False, usecols=cols_to_load):
                file_input_rows += len(chunk)
                chunk[filter_col] = chunk[filter_col].str.strip()  # strip whitespace from CSV values to match filter values reliably
    ...
    reduction = (1 - total_output_rows / total_input_rows) * 100 if total_input_rows else 0
    ...
    print(f"  {'Columns removed:':<16}{cols_removed:>{count_w},}   ({cols_removed_pct:.1f}%)")
    print(f"  {'Rows out:':<16}{total_output_rows:>{count_w},}")
    print(f"  {'Rows removed:':<16}{total_input_rows - total_output_rows:>{count_w},}   ({reduction:.1f}% reduction)")
    rule()
    ...
    if total_output_rows > EXCEL_ROW_LIMIT:
        print(f"  ⚠  {total_output_rows:,} rows exceeds Excel's worksheet limit of {EXCEL_ROW_LIMIT:,}.")
        if args.excel:
